...
const AllCarDetails = () => {
    ...
    const { allDetailsId } = useParams();
    const { user } = useAuth();
   
    useEffect(() => {
        fetch(`https://mighty-journey-16776.herokuapp.com/allcars/${allDetailsId}`)
            .then(res => res.json())
                        ...
                        <CardActionArea>
                            ...
                            <CardContent>
                                <Typography gutterBottom variant="h5" component="div">
                                    Name: {allCarDetails?.name}
                                </Typography>
                                <Typography gutterBottom variant="h6" component="div">
                                    Price: {allCarDetails?.price}
                                </Typography>
                                <Typography variant="body2" color="text.secondary">
                                    {allCarDetails?.description}
                                </Typography>
                            </CardContent>
                        </CardActionArea>
                        <CardActions>
